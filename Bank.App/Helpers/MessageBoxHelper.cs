...
{
	/// <summary>
	///     https://www.daimto.com/wpf-center-message-box-on-window/
	/// </summary>
	public static class MessageBoxHelper
    {
	    ...
	    internal static void CenterMessageBox(this Window form)
        {
        ...
        private class MessageBoxCenterHelper
        {
            ...
            {
                NativeMethods.CenterMessageCallBackDelegate callBackDelegate = CenterMessageCallBack;
                GCHandle.Alloc(callBackDelegate);

                parentFormHandle = new WindowInteropHelper(form).Handle;
                messageHook = NativeMethods.SetWindowsHookEx(5, callBackDelegate,
                        new IntPtr(NativeMethods.GetWindowLong(parentFormHandle, -6)),
            ...
            private int CenterMessageCallBack(int message, int wParam, int lParam)
            {
                ...
                if (message == 5)
                {
                    ...
                    xPos = formRect.Left + (formRect.Right - formRect.Left) / 2 -
                           (messageBoxRect.Right - messageBoxRect.Left) / 2;
                    yPos = formRect.Top + (formRect.Bottom - formRect.Top) / 2 -
